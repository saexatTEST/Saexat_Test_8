...
function normalize(list: unknown): AdminRecord[] {
  ...
  return list
    ...
    .map((a) => {
      const r = a as Partial<AdminRecord>;
      return {
        id: r.id ?? `adm_${Math.random().toString(36).slice(2, 9)}`,
        name: r.name ?? '',
        surname: r.surname ?? '',
        idNumber: r.idNumber ?? '',
        username: r.username ?? '',
...
export function AdminsProvider({ children }: { children: ReactNode }) {
  ...
  const addAdmin: AdminsContextValue['addAdmin'] = useCallback(
    (input) => {
  ...
  const updateAdmin: AdminsContextValue['updateAdmin'] = useCallback(
    (id, patch) => {
      setData((prev) =>
        normalize(prev).map((a) =>
          a.id === id
            ? {
                ...a,
                ...patch,
  ...
  const removeAdmin: AdminsContextValue['removeAdmin'] = useCallback(
    (id) => {
